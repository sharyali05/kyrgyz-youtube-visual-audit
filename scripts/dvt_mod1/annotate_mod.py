...
def predict_raw_fixed(self, frames: np.ndarray):
    with torch.no_grad():
        device = next(self.parameters()).device
        inp = torch.from_numpy(frames).to(device)
        single_frame_pred, all_frames_pred = self(inp)
...
def annotate_shots(vid_filename, model, visualize=True):
    ...
    # Pre-calculate frame count if possible, or use a list
    frames = []
    for frame in container.decode(video=0):
        # Convert frame to numpy array (RGB)
        img = frame.to_ndarray(format='rgb24')
        # Resize to the tiny 27x48 required by TransNetV2
...
def yield_video(video_path):
    ...
    # Optional: Use multiple threads for faster software decoding
    stream.thread_type = "AUTO"

    # container.decode(video=0) yields DecodedFrame objects
    for i, frame in enumerate(container.decode(video=0)):

        # 1. Convert PyAV frame to RGB Numpy array
        # format='rgb24' ensures it's in the format you expect
        img = frame.to_ndarray(format='rgb24')

        # 2. Get the frame index
        # We can use the loop enumerator 'i' as the frame count
        frame_index = i

        # 3. Get the timestamp in milliseconds
        # PyAV frame.time is in seconds; multiply by 1000 for msec
        msec = frame.time * 1000

        yield img, frame_index, msec
